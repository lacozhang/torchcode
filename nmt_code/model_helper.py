...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 19 17:47:52 2018
...
def get_initializer(init_op, seed=None, init_weight=None):
    """Create an initializer, init_weight only used for uniform"""
    if init_op == "uniform":
        assert init_weight
...
def create_eval_model(model_creator, hparams, scope=None, extra_args=None):
    """Create train graph, model, src/tgt file placeholder, and iterator"""
    src_vocab_file = hparams.src_vocab_file
    tgt_vocab_file = hparams.tgt_vocab_file
    graph = tf.Graph()
...
def _create_pretrained_emb_from_text(
        vocab_file, embed_file, num_trainable_tokens=3, dtype=tf.float32,
        scope=None):
    """Load pretrained embedding from embed_file, return an embedding matrix
    
    
    Args:
        embed_file: path to Glove format embedding
        num_trainable_tokens: Make the first n tokens in the vocab file as 
        trainable. Default is 3, which is "<unk>", "<s>", "</s>".
    """
    ...
    emb_dict, emb_size = vocab_utils.load_embed_text(embed_file)
    ...
    emb_mat_constant = tf.slice(emb_mat, [num_trainable_tokens, 0], [-1, -1])
    with tf.variable_scope(scope or "pretrain_embeddings", dtype=dtype) as scope:
        with tf.device(_get_embed_device(num_trainable_tokens)):
            emb_mat_var = tf.get_variable(
                    "emb_mat_var", [num_trainable_tokens, emb_size])
    return tf.concat([emb_mat_var, emb_mat_constant], 0)

def _create_or_load_embed(embed_name, vocab_file, embed_file, 
                          vocab_size, embed_size, dtype):
    if vocab_file and embed_file:
        embedding = _create_pretrained_emb_from_text(vocab_file, embed_file)
    else:
...
def _single_cell(unit_type, num_units, forget_bias, dropout, mode,
                 residual_connection=False, device_str=None, residule_fn=None):
    ...
    if unit_type == "lstm":
        ...
        single_cell = tf.contrib.rnn.BasicLSTMCell(num_units=num_units,
    ...
    if residual_connection:
        single_cell = tf.contrib.rnn.ResidualWrapper(single_cell, residule_fn)
        utils.print_out("  %s"%(type(single_cell).__name__), new_line=False)
    
    #Device Wrapper
    if device_str:
        single_cell = tf.contrib.rnn.DeviceWrapper(
                cell=single_cell, device=device_str)
...
def create_rnn_cell(unit_type, num_units, num_layers, num_residual_layers,
    ...
    Args:
        ...
        num_units: the depth of unit.
        num_layers: #cells.
        num_residual_layers: Number of residual layers from top to bottom. For
            example, if `num_layers=4` and `num_residual_layers=2`, then last 2
            RNN cells in the returned list will be wrapped with `ResidualWrapper`.
        forget_bias: the initial forget bias of RNN cell.
