...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 12 11:31:22 2018
General utility fuction
@author: zhangyu
"""
...
def check_tensorflow_version():
    ...
    if tf.__version__ < min_tf_version:
        raise EnvironmentError("Tensorflow versioin must >= %s"%(min_tf_version))
...
def print_time(msg, start_time):
    print("%s, time %ds, %s."%(msg, (time.time()-start_time), time.time()))
    sys.stdout.flush()
    return time.time()
...
def load_hparams(model_dir):
    """Load hyper-params from an existing model dir."""
...
def maybe_parse_standard_hparams(hparams, hparams_path):
    ...
    if tf.gfile.Exists(hparams_path):
        print_out("# Loading standard params from %s"%(hparams_path))
...
def save_hparams(out_dir, hparams):
    ...
    with codecs.getreader("utf-8")(tf.gfile.GFile(hparams_file, "wb")) as f:
...
def add_summary(summary_writer, global_step, tag, value):
    """Add summary to the current summary writer.
...
def format_text(words):
    """Convert a sentence words into sentence."""
    if (not hasattr(words, "__len__") and
...
def format_spm_text(symbols):
    """Decode a sentence in SPM format."""
